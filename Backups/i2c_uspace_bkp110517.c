...
int main()
{
	...
	int MPU6050_Register_[16];	//this one contains all MPU6050 Registers
	...
	while(1) {
		// Read All Values
		for(i=59; i<=72;i++)
			MPU6050_Register_[i] = i2c_smbus_read_byte_data(i2c_dev_node, i);

		if (MPU6050_Register_[59] < 0) {		// Check if there is any data
			perror("I2C Read operation failed.");
			exit(3);
		}
		GForcesX=(MPU6050_Register_[59] << 8) | MPU6050_Register_[60];
//		GForcesX=-(GForcesX-32768)/16384;
		printf("GForcesX = %f\n", GForcesX);

		GForcesY=(MPU6050_Register_[61] << 8) | MPU6050_Register_[62];
//		GForcesY=-(GForcesY-32768)/16384;
		printf("GForcesY = %f\n", GForcesY);

		GForcesZ=(MPU6050_Register_[63] << 8) | MPU6050_Register_[64];
//		GForcesZ=-(GForcesZ-32768)/16384;
		printf("GForcesZ = %f\n", GForcesZ);

		Temp= (MPU6050_Register_[65] << 8) | MPU6050_Register_[66]; 
